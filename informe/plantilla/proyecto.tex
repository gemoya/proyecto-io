...
Utilizamos como input inicial la siguiente tabla donde se muestra que músico toca que pieza musical:

\begin{center}
	\begin{tabular}{c|ccccccccc}
	Músico \textbackslash Pieza & 1 & 2 & 3 & 4 & 5 & 6 & 7 & 8 & 9 \\ \hline
	1 & 1 & 1  & 0 & 1 & 0 & 1 & 1 & 0 & 1\\
	2 & 1 & 1  & 0 & 1 & 1 & 1 & 0 & 1 & 0\\
	3 & 1 & 1  & 0 & 0 & 0 & 0 & 1 & 1 & 0\\
	4 & 1 & 0  & 0 & 0 & 1 & 1 & 0 & 0 & 1\\
...
Solo mostraremos los valores que no son nulos de los que decidimos que son importantes para analizar. (Cuadro \ref{table1})
\begin{table}
	\begin{align*}
		s_{9,1}&        1.000000\\        
		s_{9,2}&        1.000000\\          
...
Para el caso del modelo extendido utilizamos la misma tabla del caso estándar pero ahora le asignamos costos a cada músico
\begin{center}
	\begin{tabular}{cc}
	Músico & Costo \\ \hline
	1 & 1\\
...
El modelo extendido se demoró un poco más que el modelo normal, con un tiempo de ejecución de \textbf{3 hr : 10 min : 02 seg}, un costo mínimo de \textbf{32}. 

Se muestran las mismas variables que el caso estándar, pero del modelo extendido. (Cuadro \ref{table2})
...
Se observa que el tiempo de ejecución es muy grande, más que nada por la cantidad de variables que tiene nuestro modelo. El resultado obtenido es el óptimo, pero la mejora no es tanto (por lo menos en este caso), ya que el valor inicial es 34 [u. de costo] y 2 más que el valor óptimo. 

El orden que le dio nuestro modelo a las piezas musicales es el siguiente:

$$ 9\rightarrow4\rightarrow2\rightarrow1\rightarrow5\rightarrow6\rightarrow8\rightarrow3 $$

\subsection{Análisis General}
	El tiempo de ejecución de los dos modelos se debe a la gran cantidad de variables utilizamos, y al método que utiliza el software \emph{LINGO} para resolver el problema, que en este caso es \emph{Branch and Bound} que depende mucho de en que orden se va a recorriendo el árbol de resultados. 
	
	En relación al orden de las piezas, se observan diferencias en cómo nuestros distintos modelos organizan las piezas, pero esto se debe a que cada modelo busca cosas distintas, uno minimiza el tiempo de espera y el segundo tiene que minimizar el tiempo de espera pero teniendo en cuenta el costo asociado a esperar ese tiempo, para minimizar este costo.
